...
async def _apply_location_device(session: AsyncSession, frame: ParsedFrame, parsed: dict) -> None:
    ...
    row.last_loc_at = datetime.utcnow()
...
async def process_inbound_frame(
    ...
    summary_json = summary_to_json(parsed)
    ...
    if cmd in ("UD", "AL", "GETLOC", "CLOCKIN", "CLOCKOUT", "WT"):
        await _apply_location_device(session, frame, parsed)
